...
public class MaximumConsecutiveGap {
    ...
    private static  int maximumGap(final List<Integer> arr){
        ...
        Integer maxBucket[] = new Integer[n - 1];
        Integer minBucket[] = new Integer[n - 1];
        ...
        for (int i = 0; i < n; i++) {
            if (arr.get(i) == maxVal || arr.get(i) == minVal) {
                continue;
            }

            // Finding index of bucket.
            int index =  (int) ((arr.get(i) - minVal) / delta);

            // Filling/Updating maximum value of bucket
            if (maxBucket[index] == Integer.MIN_VALUE) {
                maxBucket[index] = arr.get(i);
            } else {
                maxBucket[index] = Math.max(maxBucket[index], arr.get(i));
            }

            // Filling/Updating minimum value of bucket
            if (minBucket[index] == Integer.MAX_VALUE) {
                minBucket[index] = arr.get(i);
            } else {
                minBucket[index] = Math.min(minBucket[index], arr.get(i));
            }
        }
